...
const AccordionItem = ({ item, setOpenQuestion, openQuestion }) => {
  ...
  return (
    <div className="accordion-item">
      <button className="accordion-item__bar" onClick={() => handleQuestionClick(id)}>
        <p className="text-med accordion-item__question">{question}</p>
        <img className="accordion-icon accordion-item__icon" src={arrow} alt="arrow" />
      </button>
      <div className={`accordion-item__answer ${open ? "visible" : ""}`}>
        <div className="accordion-item__heading-container">
          ...
          <img className="accordion-icon accordion-item__answer-icon" src={arrowRight} alt="arrow" />
        </div>
